...
def create_pdf(df, total_miles_relief, uniform_amount, lang):
    ...
    if lang == "EN":
        line1 = f"Generated: {today_str}  |  Tax Year: {tax_year}"
    else:
        line1 = f"Wygenerowano: {today_str}  |  Rok podatkowy: {tax_year}"
    pdf.cell(0, 7, text=line1, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.ln(4)
    COL = [24, 30, 30, 30, 20, 26, 30]
    if lang == "EN":
        headers = ["Date", "From", "To", "Purpose", "Miles", "Agency(p)", "Relief(GBP)"]
    else:
        headers = ["Data", "Skad", "Dokad", "Cel", "Mile", "Agencja(p)", "Ulga(GBP)"]
    ...
    if lang == "EN":
        total_label = f"TOTAL RELIEF / EXPENSE: GBP {final_total:.2f}"
    else:
        total_label = f"LACZNA ULGA / KOSZT: GBP {final_total:.2f}"
    ...
    if lang == "EN":
        disclaimer = ("This report is for estimation purposes only. "
                      "HMRC makes the final decision on all tax relief claims.")
    else:
        disclaimer = ("Raport ma charakter szacunkowy. "
                      "Ostateczna decyzja o przyznaniu ulgi nalezy do HMRC.")
...
.stButton>button {
    ...
    border-radius: 8px !important; border: 2px solid #D4AF37 !important;
...
with col_m:
    ...
    logo_candidates = [
        ...
        "logo.png.png",
        "acountingpro.png", "acountingpro.jpg",
        "AcountingPro.png", "AcountingPro.jpg",
        "ACountingPro.png", "ACountingPro.jpg",
    ]
    logo_found = False
    ...
    if not logo_found:
        st.markdown(
            "<h1 style='text-align:center;color:#002147;'>A Counting Pro</h1>",
            unsafe_allow_html=True,
        )
    st.markdown(
        "<h3 style='text-align:center;color:#D4AF37;font-style:italic;'>"
        "Financial health is mental wealth</h3>",
        unsafe_allow_html=True,
    )
    if EN:
        sub = "Support for Care, Cleaning & Warehouse Professionals"
    else:
        sub = "Wsparcie dla Care, Cleaning i Magazynow"
    st.markdown(
        f"<p style='text-align:center;color:#002147;'><b>{sub}</b></p>",
        unsafe_allow_html=True,
    )
...
with tab1:
    ...
    with col_in:
        ...
        emp_status = st.radio(
            "Employment Status" if EN else "Status zatrudnienia",
            [emp_paye, emp_se],
        )
        st.write("---")
        ...
        m_raw = st.number_input(
            "Miles (one way)" if EN else "Mile (w jedna strone)",
        ...
        if is_paye:
            ...
            if a > 45:
                st.warning(
                    "Agency rate above 45p - employer reimburses above HMRC limit. Relief = 0."
                    if EN else
                    "Stawka powyzej 45p - pracodawca zwraca powyzej limitu HMRC. Ulga = 0."
                )
            hmrc_total = m * 0.45
            ...
            a = 0.0
            st.info(
                "As Self-Employed you deduct the full 45p per mile as a business expense."
                if EN else
                "Jako samozatrudniony odliczasz pelne 45p za mile jako koszt firmowy."
            )
            hmrc_total = m * 0.45
            agency_total = 0.0
            relief = hmrc_total
    ...
    with col_math:
        ...
        st.caption(
            ...
            if EN else
            "Szacunek orientacyjny. Ostateczna decyzja nalezy do HMRC."
        )
...
with tab2:
    st.warning(
        "Download your PDF & CSV before closing this page - data will not be saved after you leave."
        if EN else
        "Pobierz PDF i CSV zanim zamkniesz strone - dane nie beda dostepne po wyjsciu z aplikacji."
    )
    ...
    if not st.session_state.history.empty:
        ...
        add_uniform = st.checkbox(
            ...
            if EN else
            "Dolicz roczny ryczalt za pranie firmowego uniformu w domu"
        )
        if add_uniform:
            sector_options = {
            ...
            if sector_options[selected] is None:
                uniform_amount = st.number_input(
                    "Uniform allowance (GBP)" if EN else "Kwota ryczaltu (GBP)",
                    min_value=0.0, value=60.0, step=1.0,
                )
            else:
                uniform_amount = sector_options[selected]
                st.info(
                    f"{'Flat rate for this sector' if EN else 'Ryczalt dla tej branzy'}: "
                    f"**GBP {uniform_amount:.0f}**"
                )
            st.caption(
                "Only if employer does NOT reimburse laundry. No receipts needed for flat rate."
                if EN else
                "Tylko jesli pracodawca NIE zwraca kosztow prania. Przy ryczalcie nie trzeba paragonow."
            )
        else:
            uniform_amount = 0.0
        ...
        c1.metric(
            "Total Miles" if EN else "Suma Mil",
            f"{st.session_state.history['Miles'].sum():.1f}",
        )
        c2.metric("Total Relief" if EN else "Suma Ulgi", f"GBP {final_total:.2f}")
        ...
        with dl1:
            pdf_bytes = create_pdf(
                st.session_state.history, total_m_relief, uniform_amount, lang
            )
            st.download_button(
        ...
        st.info(
            "No data yet. Add your trips in the Calculator tab!"
            if EN else
            "Brak danych. Dodaj przejazdy w zakladce Kalkulator!"
        )
...
st.markdown(
    ...
    f"(c) {date.today().year} A Counting Pro | Financial health is mental wealth</p>",
    unsafe_allow_html=True,
)
